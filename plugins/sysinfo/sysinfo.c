...
sysinfo_set_pref (char *key, char *value)
{
	if (!key || !key[0])
	{
		hexchat_printf (ph, _("Sysinfo: Valid settings are: announce and hide_* for each piece of information. e.g. hide_os. Without a value it will show current (or default) setting.\n"));
		return;
	}
